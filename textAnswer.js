...
module.exports = ({name, time, price, templateName }) => {
    ...
    return `Здравствуйте${name ? ' '+name : ''}! Готов ${templateName === '9' ? 'приступить ' : 'приехать '} ${time[0] === '0' ? '' : data.answer.time[Number(time[0])] + ' или '}в удобное для Вас время и дату${time[1] === '1' ? ', на данный момент свободен' : ''}. Согласен на ${price}руб.
